...
import pe.idat.taxikotlin.databinding.ActivityTripInfoBinding
import pe.idat.taxikotlin.models.Prices
import pe.idat.taxikotlin.providers.ConfigProvider
...
class TripInfoActivity : AppCompatActivity(), OnMapReadyCallback, Listener, DirectionUtil.DirectionCallBack {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d("LOCATION", "Destination Lat: ${destinationLatLng?.latitude}")
        Log.d("LOCATION", "Destination Lng: ${destinationLatLng?.longitude}")
    ...
    private fun getPrices(distance: Double, time: Double){
        configProvider.getPrices().addOnSuccessListener { document ->
            if (document.exists()){
                val prices = document.toObject(Prices::class.java) //info del documento prices
                ...
                var minTotal = total - prices?.difference!! //descontando 2 soles al total
                var maxTotal = total + prices?.difference!! // sumando 2 soles al total

                val minTotalString = String.format("%.1f", minTotal)
                val maxTotalString = String.format("%.1f", maxTotal)

                binding.textViewPrice.text = "S/.$minTotalString - S/.$maxTotalString"
    ...
    private fun addDestinationMarker(){
        markerOrigin = googleMap?.addMarker(MarkerOptions().position(destinationLatLng!!).title("Destino")
    ...
    override fun pathFindFinish(
        ...
        distance = if (distance < 1000.0) 1000.0 else distance // se queda en 1km si es menos de 1km
